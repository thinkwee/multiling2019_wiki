...
def get_data(
    ...
    cls = None
    meta = None
    for args in enumerate(zip(*all_args)):
        if is_cls:
            if is_meta:
        ...
        if len(orig_tokens) > 50 or len(labels) > 50 or len(orig_tokens) < 3 or len(labels) < 3:
            continue
        if len(orig_tokens) != len(labels):
            if len(labels) > len(orig_tokens):
                labels = labels[:len(orig_tokens)]
            else:
                orig_tokens = orig_tokens[:len(labels)]
...
def get_bert_data_loaders(train, valid, vocab_file, batch_size=16, cuda=True, is_cls=False,
    ...
    valid = pd.read_csv(valid, header=None)
