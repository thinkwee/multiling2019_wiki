...
def train_step(dl, model, step_total, optimizer, lr_scheduler=None, clip=None, num_epoch=1):
    model.train()
    epoch_loss = 0
    step = 0
    ...
    for batch in pr:
        ...
        epoch_loss += loss
        pr.set_description("train loss: {}".format(epoch_loss / step))
        update = 'append' if step_total>1 else None
#         viz.line(
#             X=torch.FloatTensor([step_total]),
#             Y=torch.FloatTensor([epoch_loss / step]),
#             win='loss',
#             update=update,
#             opts=opts_loss,
#             name='train'
    ...
    def fit(self, epochs=100, resume_history=True, target_metric="f1"):
        ...
        try:
            ...
            for _ in range(epochs):
                self.epoch += 1
                step_total = self.fit_one_cycle(self.epoch, step_total, target_metric)
        except KeyboardInterrupt:
            pass

    def fit_one_cycle(self, epoch, step_total, target_metric="f1"):
        steps = train_step(self.data.train_dl, self.model, step_total, self.optimizer, self.lr_scheduler, self.clip, epoch)
